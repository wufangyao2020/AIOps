# A股海外产业信号雷达｜2026-09-18 08:43（北京时间）

> 海外信号只负责提前发现变化，不直接生成买卖指令。必须再用公司公告、季度扣非利润、经营现金流或相对强弱交叉验证。

## 今日摘要

- 触发P1：4条；P2：5条；P3：0条。
- 数据源成功：24条；失败：13条。
- 最强信号：台积电月营收同比（P1）；布伦特原油现货（P1）；WTI原油现货（P1）；荣昌生物海外临床状态（P1）；费城半导体指数（P2）。

## 触发信号

|级别|信号|最新值/事件|单周期|五周期|方向|来源|
|---|---|---:|---:|---:|---|---|
|P1|台积电月营收同比|53.30|19.24%|17.92%|上行|TSMC investor relations|
|P1|布伦特原油现货|130.80|7.88%|23.26%|上行|FRED|
|P1|WTI原油现货|107.02|4.49%|13.60%|上行|FRED|
|P1|荣昌生物海外临床状态|1条事件|—|—|下行|A|
|P2|费城半导体指数|11,599.49|3.14%|-0.13%|上行|Yahoo public chart endpoint|
|P2|ICE棉花期货|82.68|2.78%|-2.32%|上行|Yahoo public chart endpoint|
|P2|美国10年期国债收益率|5.01|0.20%|3.73%|上行|FRED|
|P2|欧洲纯电动车注册份额|20.70|—%|—%|上行|ACEA|
|P2|USDA全球农产品供需报告|1条事件|—|—|待判定|A|

## A股优先复核

|代码|公司|方向分|正向|负向|待判定|主要触发|最终20|
|---|---|---:|---:|---:|---:|---|---|
|600938|中国海油|5.40|5.40|0.00|0.00|布伦特原油现货(P1/A)；WTI原油现货(P1/B)|否|
|601857|中国石油|4.35|4.35|0.00|0.00|布伦特原油现货(P1/A)；WTI原油现货(P1/B)|否|
|300308|中际旭创|4.20|4.20|0.00|0.00|台积电月营收同比(P1/A)；费城半导体指数(P2/C)|否|
|300502|新易盛|4.20|4.20|0.00|0.00|台积电月营收同比(P1/A)；费城半导体指数(P2/C)|否|
|300476|胜宏科技|3.78|3.78|0.00|0.00|台积电月营收同比(P1/A)；费城半导体指数(P2/C)|否|
|002463|沪电股份|3.78|3.78|0.00|0.00|台积电月营收同比(P1/A)；费城半导体指数(P2/C)|否|
|600256|广汇能源|3.60|3.60|0.00|0.00|WTI原油现货(P1/B)；布伦特原油现货(P1/B)|否|
|601138|工业富联|3.36|3.36|0.00|0.00|台积电月营收同比(P1/A)；费城半导体指数(P2/C)|否|
|688008|澜起科技|3.15|3.15|0.00|0.00|台积电月营收同比(P1/B)；费城半导体指数(P2/C)|否|
|002916|深南电路|3.15|3.15|0.00|0.00|台积电月营收同比(P1/B)；费城半导体指数(P2/C)|否|
|688331|荣昌生物|-3.00|0.00|3.00|0.00|荣昌生物海外临床状态(P1/A)|是|
|603986|兆易创新|2.73|2.73|0.00|0.00|台积电月营收同比(P1/B)；费城半导体指数(P2/C)|否|
|301308|江波龙|2.73|2.73|0.00|0.00|台积电月营收同比(P1/B)；费城半导体指数(P2/C)|否|
|688012|中微公司|2.31|2.31|0.00|0.00|台积电月营收同比(P1/B)；费城半导体指数(P2/C)|否|
|002371|北方华创|2.31|2.31|0.00|0.00|台积电月营收同比(P1/B)；费城半导体指数(P2/C)|否|
|601111|中国国航|-2.10|0.00|2.10|0.00|布伦特原油现货(P1/C)|否|
|600115|中国东航|-2.10|0.00|2.10|0.00|布伦特原油现货(P1/C)|否|
|600029|南方航空|-2.10|0.00|2.10|0.00|布伦特原油现货(P1/A)|否|
|600359|新农开发|1.50|1.50|0.00|0.00|ICE棉花期货(P2/B)|否|
|600540|新赛股份|1.50|1.50|0.00|0.00|ICE棉花期货(P2/B)|否|
|002048|宁波华翔|1.50|1.50|0.00|0.00|欧洲纯电动车注册份额(P2/A)|是|
|603997|继峰股份|1.30|1.30|0.00|0.00|欧洲纯电动车注册份额(P2/A)|否|
|600699|均胜电子|1.30|1.30|0.00|0.00|欧洲纯电动车注册份额(P2/A)|否|
|300693|盛弘股份|1.26|1.26|0.00|0.00|台积电月营收同比(P1/B)；费城半导体指数(P2/C)|否|
|601339|百隆东方|-1.20|0.00|1.20|0.00|ICE棉花期货(P2/B)|否|
|002042|华孚时尚|-1.20|0.00|1.20|0.00|ICE棉花期货(P2/B)|否|
|002311|海大集团|0.00|0.00|0.00|1.10|USDA全球农产品供需报告(P2/B)|否|
|600028|中国石化|1.05|1.05|0.00|0.00|布伦特原油现货(P1/B)|否|
|000726|鲁泰A|-1.00|0.00|1.00|0.00|ICE棉花期货(P2/B)|否|
|000876|新希望|0.00|0.00|0.00|1.00|USDA全球农产品供需报告(P2/B)|否|

## 最终20只的海外敏感度

|代码|公司|敏感度|重点变量|说明|
|---|---|---|---|---|
|688331|荣昌生物|中高|荣昌生物海外临床状态、艾伯维SEC重大披露|nan|
|000612|焦作万方|中高|CME铝期货|nan|
|600267|海正药业|低|无强直接变量|主要依赖国内经营与公司公告，不应强行用海外数据解释。|
|603091|众鑫股份|中高|美国纸浆模塑/食品包装贸易政策、美元兑人民币|nan|
|603162|海通发展|中高|波罗的海干散货指数 BDI、海岬型船指数 BCI、巴拿马型船指数 BPI、超灵便型船指数 BSI|nan|
|002606|大连电瓷|中|美国输电与变压器政策|nan|
|601002|晋亿实业|中|欧洲纯电动车注册份额|nan|
|600566|济川药业|低|无强直接变量|主要依赖国内经营与公司公告，不应强行用海外数据解释。|
|603605|珀莱雅|低|美国10年期国债收益率|海外利率或风险偏好只构成弱影响，核心仍是国内经营与资产质量。|
|605020|永和股份|中高|美国HFC/制冷剂政策|nan|
|002805|丰元股份|中|美国电池与电动车贸易政策|nan|
|600351|亚宝药业|低|无强直接变量|主要依赖国内经营与公司公告，不应强行用海外数据解释。|
|605090|九丰能源|中高|LPG航运指数 BLPG、美元兑人民币、LNG航运指数 BLNG|nan|
|600663|陆家嘴|低|美国10年期国债收益率|海外利率或风险偏好只构成弱影响，核心仍是国内经营与资产质量。|
|002274|华昌化工|中|Henry Hub天然气现货|nan|
|000833|粤桂股份|中高|COMEX白银期货、ICE原糖期货|nan|
|000737|北方铜业|中高|COMEX铜期货|nan|
|600717|天津港|中|波罗的海干散货指数 BDI、全球集装箱运价指数 FBX|nan|
|600872|中炬高新|中|CBOT大豆期货、美元兑人民币|nan|
|002048|宁波华翔|中高|欧洲纯电动车注册份额、欧元兑人民币、美元兑人民币|nan|

## 新事件

- **荣昌生物海外临床状态｜2026-09-18｜负向**：NCT05306574状态为TERMINATED：A Study of Telitacicept for the Treatment of Moderately to Severely Active Systemic Lupus Erythematosus (REMESLE-1)  
  持续性负面状态；在状态恢复或替代试验明确前维持风险提示。  
  来源：https://clinicaltrials.gov/study/NCT05306574
- **USDA全球农产品供需报告｜2026-09-18｜待判定**：USDA WASDE页面出现更新  
  需比较小麦、粗粮、油籽、糖和棉花的产量及库存消费比修正。  
  来源：https://www.usda.gov/about-usda/general-information/staff-offices/office-chief-economist/commodity-markets/wasde-report

## 数据健康

- `bci`：bci absent from Baltic page
- `bpi`：bpi absent from Baltic page
- `bsi`：bsi absent from Baltic page
- `bdti`：bdti absent from Baltic page
- `bcti`：bcti absent from Baltic page
- `blng`：blng absent from Baltic page
- `blpg`：blpg absent from Baltic page
- `fbx`：fbx absent from Baltic page
- `bai00`：bai00 absent from Baltic page
- `sia_sales_yoy`：SIA official pages did not expose a parseable latest global-sales release
- `acea_bev_share`：ACEA official pages did not expose a parseable passenger-registration release
- `sec_abbvie`：GET failed after 2 attempts: https://www.sec.gov/cgi-bin/browse-edgar: 403 Client Error: Forbidden for url: https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0001551152&type=&owner=exclude&count=40&output=atom
- `sec_hyperscalers`：GET failed after 2 attempts: https://www.sec.gov/cgi-bin/browse-edgar: 403 Client Error: Forbidden for url: https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0000789019&type=&owner=exclude&count=40&output=atom

## 执行纪律

1. P1只表示开盘前必须复核，不表示无条件追涨。
2. A类映射优先；B类需要合同、套保和产品结构确认；C类只作宏观或价格确认。
3. 商品必须看利润差：铝价同时看氧化铝和电力，油价同时看炼化价差，运价同时看船型与实际TCE。
4. 同一信号连续增强但公司不再跑赢行业，可能已经定价；基本面增强而价格尚未反应，才可能形成预期差。
